    ...
    .props({
        ...
        __getMissing(name){
            if(!this._instances.hasOwnProperty(name)){
                this._instances[name] = getInstanceFromAncestors(this.parentEnvironment, name);
...
const getInstanceFromAncestors = (environment, name) => {
    if(environment){
        if(environment._instances.hasOwnProperty(name)){
            return environment._instances[name];
        }
        return getInstanceFromAncestors(environment.parentEnvironment, name);
    }
};

export const createEnvironment = (...args) => Environment.new();
