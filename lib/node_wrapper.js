...
export const defineNodeWrapper = overload({
    ['string, object, function'](name, options, definition){
        NodeWrapper.register(name).define(dsl => dsl
            .tap(Class => {
                if(options.selector){
                    Class.define(dsl => dsl
    ...
    ['string, function'](name, definition){
        defineNodeWrapper(name, {}, definition);
    }
});