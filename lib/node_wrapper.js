...
export const NodeWrapper = Base.extend().define(dsl => dsl
    .include(Registrable)
    .tap(Class => {
...
export const defineWidget = overload({
    ['string, object, object|function'](name, options, props){
        const { extends: _extends, selector } = options;

        if(typeof props == 'function'){
            props = {
                initialize(...args){
                    this.__proto.initialize.call(this, ...args);
                    props.call(this, this);
                }
            };
        }

        (_extends ? NodeWrapper.register(_extends) : NodeWrapper).register(name).define(dsl => {
            if(selector){
                dsl.selector(selector);
            }
            dsl.props(props);
    ...
    ['string, object|function'](name, props){
        defineWidget(name, {}, props);
    }
});